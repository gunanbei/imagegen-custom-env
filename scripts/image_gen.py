...
def _image_item_bytes(item: Any) -> bytes:
    """Decode an Images API item returned as base64 or as a downloadable URL."""
    image_b64 = getattr(item, "b64_json", None)
    if image_b64:
        try:
            return base64.b64decode(image_b64, validate=True)
        except (ValueError, TypeError) as exc:
            _die(f"Invalid b64_json image response: {exc}")

    image_url = getattr(item, "url", None)
    if not image_url:
        _die("Image response item contains neither b64_json nor url.")
    if image_url.startswith("data:"):
        match = re.fullmatch(r"data:image/[^;]+;base64,(.+)", image_url, re.DOTALL)
        if not match:
            _die("Unsupported data URL image response.")
        try:
            return base64.b64decode(match.group(1), validate=True)
        except (ValueError, TypeError) as exc:
            _die(f"Invalid data URL image response: {exc}")
    ...
    if not raw:
        _die("Downloaded image URL returned an empty response.")
    return raw


def _decode_and_write(items: List[Any], outputs: List[Path], force: bool) -> None:
    for idx, item in enumerate(items):
        ...
        out_path.write_bytes(_image_item_bytes(item))
